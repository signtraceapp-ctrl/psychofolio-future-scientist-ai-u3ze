...
const siteContentInputSchema = z.object({
  ...
  metrics: z.array(z.object({ val: metin, unit: metin.optional(), label: metin })).optional(),
...
export interface SiteContent {
  ...
  metrics?: { val: string; unit?: string; label: string }[];
  about: {
    title: string;
    intro: string;
    introSecond?: string;
...
// ── Content loader (no module-level cache) ──
    ...
    try {
      ...
      if (parsed.success && Object.keys(parsed.data).length > 0) {
        ...
        return {
          ...
          metrics: g.metrics && g.metrics.length > 0 ? (g.metrics as SiteContent["metrics"]) : defaults.metrics,
          services:
