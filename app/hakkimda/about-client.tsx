...
export function AboutClient({ content: c }: { content: SiteContent }) {
  ...
  const organizations = c.about.organizations ?? [];
      ...
      <section className="py-28 bg-bg text-fg relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-2xl space-y-16 text-center">
            ...
            {/* Callout card */}
            {(c.about.calloutTitle || c.about.calloutText) && (
              <div data-reveal className="mx-auto max-w-md p-6 rounded-[20px] bg-bg-secondary/40 border border-primary/10 shadow-sm space-y-2">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">
                  {c.about.calloutTitle ?? "Terapi Felsefem"}
                </p>
                <p className="text-sm italic text-fg-muted font-medium">
                  &ldquo;{c.about.calloutText}&rdquo;
                </p>
              </div>
            )}
            ...
            {organizations.length > 0 && (
              <div data-reveal className="mx-auto max-w-sm space-y-4">
                <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-primary">Uzmanlık Alanları</p>
                <div className="space-y-3">
                  {organizations.map((org, i) => (
                    <div key={i} className="flex items-center justify-center gap-3 text-sm text-fg font-medium">
